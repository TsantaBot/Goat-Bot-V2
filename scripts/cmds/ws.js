...
Example: {p}ws nature 3"
  ...
  onStart: async function ({ api, event, args }) {
    if (args.length < 1) {
      api.sendMessage('Tsy mety.
*Mot clé 1 ihany ampiasaina
 Arao ny Exemple eto ambany:

Ex: #ws nature 3', event.threadID, event.messageID);
      return;
    }
    ...
    if (isNaN(amount) || amount <= 0) {
      api.sendMessage('asio chiffre 3 ny farany.
Mot clé iray ihany ampiasaina

Ex: #fond Dog 3', event.threadID, event.messageID);
      return;
    }
    ...
    try {
      ...
      if (response.data.status && response.data.img.length > 0) {
        ...
        api.sendMessage({
          ...
          body: `🇲🇹 TSANTA_AI | Fond d'écran "${keyword}" `,
        }, event.threadID, (err) => {
